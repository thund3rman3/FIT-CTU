...
// Called from GPU
__device__ double distanceND_device(const double* p1, const double* p2, size_t n) {
    double res = 0.0f;
    ...
    size_t k)
{
    ...
    const double* p_coords = &d_points_coords[i * n];

    double min_dist = distanceND_device(p_coords, &d_centroids[0], n);

    for (size_t j = 1; j < k; ++j) {
        double d = distanceND_device(p_coords, &d_centroids[j * n], n);
        if (d < min_dist) {
            min_dist = d;
            closest_idx = j;
        }
...
__global__ void finalize_centroids_kernel(
    double* d_centroids,
    const double* d_new_centroids_sums,
    const int* d_cluster_counts,
    const double* d_old_centroids,
    size_t k,
    size_t n)
{
    // max k threads
    ...
    if (count > 0) { // count avg
        for (size_t d = 0; d < n; ++d) {
            d_centroids[j * n + d] = d_new_centroids_sums[j * n + d] / (double)count;
        }
    } else { // empty volume
        for (size_t d = 0; d < n; ++d) {
...
void k_means(
    ...
    const std::vector<double>& points_coords,
    ...
    int threadsPerBlock = 256)
{
    ...
    // init (Host -> Device)
    size_t points_bytes = cnt_points * n * sizeof(double);
    size_t clusters_bytes = cnt_points * sizeof(size_t);
    size_t centroids_bytes = k * n * sizeof(double);

    double *d_points_coords, *d_centroids, *d_old_centroids, *d_new_centroids_sums;
    size_t *d_points_cluster_ids;
...
void print_clusters(const std::vector<double>& points, const std::vector<size_t>& points_cluster_ids, size_t n, size_t k, size_t cnt_points) {
...
int main() {
    ...
    std::vector<double> points_double(points.size());
    for(size_t i = 0; i < points.size(); ++i) {
        points_double[i] = static_cast<double>(points[i]);
    }
    
    std::cout << "Successfully read " << points_double.size() << " values." << std::endl;


    std::vector<size_t> test_points = {10000000}; // 1 mil, 10 mil bodů
    std::vector<size_t> test_k = {100};                // 10, 100 shluků
    std::vector<size_t> test_n = {2};            // 1, 2, 8, 64 dimenzí
    std::vector<int> test_threads = {256};       // Vlákna na blok
    ...
    for (size_t cnt_points : test_points) {
        for (size_t n : test_n) {
            ...
            size_t required_doubles = cnt_points * n;
            if (required_doubles > points_double.size()) {
                std::cout << "[SKIP]: " << cnt_points 
                          << " points, " << n << " dimensions. (Need: " 
                          << required_doubles << ", Read: " << points_double.size() << ")\n";
                continue; 
            }
            ...
            for (size_t k : test_k) {
                ...
                for (int threadsPerBlock : test_threads) {
                    ...
                    k_means(k, n, cnt_points, points_double, points_cluster_ids, threadsPerBlock);
