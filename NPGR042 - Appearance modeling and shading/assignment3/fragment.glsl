...
void main() {
    ...
    float heigh_map = clamp(1.0 - add_masks, 0.0, 1.0);
    ...
    // ---- Bump mapping -------------------------------------------
    float neighbor = 1.0 / 1024.0;
    float R = getNeighbor(vUv + vec2(neighbor, 0.0)); 
    float U = getNeighbor(vUv + vec2(0.0, neighbor)); 
