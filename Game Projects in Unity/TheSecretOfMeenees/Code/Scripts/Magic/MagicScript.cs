...
public class MagicScript : MonoBehaviour
{
    ...
    public bool active;
    public Transform m_destroyableWithBlock;

    void Start()
    {
        Model = transform.GetChild(0);
        m_destroyableWithBlock = null;
    }
    void Update(){
        if(DestroySet){
        ...
        switch (SpellData.SpellType){
            ...
            case 2:
                ...
                if (SpellData.ElementType == TemplateMagic.ElementType.Earth)
                {
                    ...
                    collider.includeLayers = LayerMask.NameToLayer("Enemy");
                    collider.excludeLayers = collider.excludeLayers & collider.includeLayers;
                }
                else if (SpellData.ElementType == TemplateMagic.ElementType.Fire)
                {
                    var light = trueSpell.GetComponent<Light>();
                    light.enabled = true;
                }
                if (m_destroyableWithBlock != null)
                {
                    tmp22.OnContactDestroysBlock(m_destroyableWithBlock);
                }
    ...
    public void OnContactDestroys(Collision coll)
    {
        ...
        else if (othT.CompareTag("Lava") && SpellData.ElementType == TemplateMagic.ElementType.Water)
        {
            ...
            Instantiate(WateredLava, cont.point, Quaternion.FromToRotation(Vector3.up, cont.normal));
            Destroy(transform.gameObject);
            return;
        }
        DestroySpell(coll);
    }
    public void OnContactDestroysBlock(Transform othT)
    {
        if (othT.CompareTag("AirDestroy") && SpellData.ElementType == TemplateMagic.ElementType.Air)
            othT.AddComponent<AirDestroy>();
        else if (othT.CompareTag("FireDestroy") && SpellData.ElementType == TemplateMagic.ElementType.Fire)
        ...
        {
            Debug.Log("created");
            GameObject wL = Instantiate(WateredLava, transform);
            wL.transform.SetParent(null, true);
            wL.transform.Rotate(-90.0f, 0.0f, 0.0f, Space.Self);
            Destroy(transform.gameObject);
            return;
        }
    }
