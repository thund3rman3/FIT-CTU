...
struct Graph{
  ...
  struct Iterator {
    ...
    size_t _v = NO_VERTEX; // cislo vrcholu
  };
  ...
  private:
  bool _dir; // orientovany/ne
  std::vector<std::vector<Vertex>> _adj; // hrany/sousedi
 ...
 * which implements the breadth-first search.
 * Its arguments are:
...
size_t bfs(const Graph& G, Vertex root, std::vector<Vertex>& P, std::vector<size_t>& D) {
	std::queue<Vertex> next;
	next.emplace(root);
	std::vector<bool> visited(G.vertices(), false);
	size_t res_visited = 1;
	visited[root] = true;
