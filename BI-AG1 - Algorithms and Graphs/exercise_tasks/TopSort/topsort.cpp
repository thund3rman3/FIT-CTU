 ...
 * `std::pair<bool, std::vector<Vertex>>
...
std::pair<bool, std::vector<Vertex>> topsort(const Graph& G) {
  ...
  std::vector<std::pair<size_t , bool>> vertex_edges(G.vertices(), std::make_pair(0, false));
  //std::vector<bool> visited(G.vertices(), false);
  std::vector<Vertex> topological_order(G.vertices());
  size_t idx = 0;
  size_t cnt = 0;
  for(Vertex u: G)
	  for(Vertex v: G[u])
		  vertex_edges[v].first++;

  for(Vertex u: G)
  {
	  if(vertex_edges[u].first == 0)
	  {
		  topological_order[idx++] = u;
		  next.emplace(u);
		  vertex_edges[u].second = true;
	  }
  }
  while(!next.empty())
  {
	  Vertex elem = next.front();
	  next.pop();

	  for(Vertex e: G[elem])
	  {
		vertex_edges[e].first--;
		if(vertex_edges[e].first == 0)
		{
			topological_order[idx++] = e;
			next.emplace(e);
			vertex_edges[e].second = true;
		}
	  }
	  cnt++;
  }
  ...
  if(cnt == G.vertices())
	  return std::make_pair(true, topological_order);
  else{
	  Graph GR = G.reversed();
	  std::queue<Vertex> nextR;
	  std::vector<Vertex> visitedR;
	  Vertex elemR;
	  size_t edges = 0, idx = 0;
	  // najdi pocatek, ktery neni soucasti topologie a ma co nejvice hran
	  for(const auto& v: vertex_edges){
		  if(!v.second &&
		  	edges < G[static_cast<Vertex>(idx)].size()){
			  elemR = static_cast<Vertex>(idx);
			  edges = GR[elemR].size();
		  }
		  idx++;
	  }
	  nextR.emplace(elemR);
	  visitedR.emplace_back(elemR);

	  while(!nextR.empty())
	  {
		  elemR = nextR.front();
		  nextR.pop();
		  for(const auto& v: GR[elemR])
		  {		// SOUSEDI KTERI NEJSOU Z TOPOLOGIE A MAJI VSTUPNI HRANY
			  if(!vertex_edges[v].second && vertex_edges[v].first != 0)
			  {	// JESTLI NAJDU NAVSTIVENY VRCHOL->NASEL JSEM KRUZNICI
				  auto it = std::find(visitedR.begin(), visitedR.end(), v);
				  if(it != visitedR.end())
				  {// PREKLOPIM VYSLEDEK DO NOVEHO VEKTORU, PROTOZE MUZE NA ZACATKU ZUSTAT BORDEL
					  std::vector<Vertex> res;
					  res.emplace_back(v);
					  for(size_t i = visitedR.size() - 1; i >= 0; --i)
					  {
						  if(visitedR[i] == v)
							  return std::make_pair(false, res);
						  res.emplace_back(visitedR[i]);
					  }
				  }
				  nextR.emplace(v);
				  visitedR.emplace_back(v);
				  break;
			  }
		  }
	  }
  }
  return {};//ABY SI Pt NESTEZOVAL
}
