...
void optimize_lists(S_LISTS *lists, SHOP *shop)
{
    ...
    for (int a = 0; a <= lists->lists_count; a++)
    {
        ...
        size_t ns_index = 0;
        OPT opt = {20, 0, (O_LIST *)malloc(opt.optimized_max * sizeof(O_LIST))};
        ...
        for (int j = 0; j < lists->list[a].product_count; j++)
        {
            ...
            for (int i = 0; i < shop->regal_count; i++)
            {
                ...
                if (cmp == 0)
                {
                    ...
                    {
                        opt.optimized_max *= 2;
                        opt.opt_list = (O_LIST *)realloc(opt.opt_list, opt.optimized_max * sizeof(O_LIST));
                    }
        ...
        for (int i = 0; i < opt.optimized_count; i++)
        {
            free(opt.opt_list[i].list_product);
            free(opt.opt_list[i].regal_product);
        }
        free(opt.opt_list);
...
void get_shopping_lists(S_LISTS *lists)
{
    ...
    int cnt = 0;
    int nl = 1;
    lists->list[lists->lists_count].product_count = 0;
    ...
    while ((state = getline(&str, &product_len, stdin)) != EOF)
    {
        ...
        {
            lists->lists_max *= 2;
            lists->list = (S_LIST *)realloc(lists->list, lists->lists_max * sizeof(S_LIST));
            for (int i = (lists->lists_max) / 2; i < lists->lists_max; i++)
            {
                lists->list[i].products = (char **)malloc(lists->list->product_max * sizeof(char *));
            }  
        }

        remove_LF(str);

        if (lists->list[lists->lists_count].product_count >= lists->list[lists->lists_count].product_max - 1)
        {
            lists->list[lists->lists_count].product_max *= 2;
            lists->list[lists->lists_count].products = (char **)realloc(lists->list[lists->lists_count].products, lists->list[lists->lists_count].product_max * sizeof(char *));
        }
...
int main(void)
{
    ...
    shop.regal_count = 0;
    shop.regal_size = 200;
    shop.regals = (REGAL *)malloc(shop.regal_size * sizeof(REGAL));
    ...
    lists.lists_max = 20;
    lists.lists_count = 0;
    lists.list = (S_LIST *)malloc(lists.lists_max * sizeof(S_LIST));
